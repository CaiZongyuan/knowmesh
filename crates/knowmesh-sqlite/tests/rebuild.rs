...
fn preview_checks_runtime_references_without_creating_a_candidate() {
    ...
    for file in ["knowledge/nodes/model-a.md", "knowledge/nodes/dataset-b.md", "knowledge/syntheses/comparison.md"] {
        fs::remove_file(temp.path().join(file)).unwrap();
    }
    let backend = SqliteRebuilder::new(&workspace).unwrap();
    let input = RebuildInput { dry_run: true, ..Default::default() };
    assert_eq!(rebuild::execute(&workspace, &backend, &input).unwrap_err().code, "RUNTIME_REFERENCE_MISSING");
    assert!(!temp.path().join(".knowmesh/index.next.sqlite3").exists());
    let report = rebuild::execute(&workspace, &backend, &RebuildInput { discard_runtime: true, ..input }).unwrap();
...
fn discard_does_not_override_workspace_identity_or_version_checks() {
    ...
    store.bind_workspace(&WorkspaceId::new(), &snapshot.schema_hash).unwrap();
    drop(store);
    let backend = SqliteRebuilder::new(&workspace).unwrap();
    let input = RebuildInput { yes: true, discard_runtime: true, ..Default::default() };
    assert_eq!(rebuild::execute(&workspace, &backend, &input).unwrap_err().code, "WORKSPACE_ID_MISMATCH");
    let db = rusqlite::Connection::open(path).unwrap();
    db.pragma_update(None, "user_version", 999).unwrap();
    drop(db);
    assert_eq!(rebuild::execute(&workspace, &backend, &input).unwrap_err().code, "UNSUPPORTED_DATABASE_VERSION");
}
...
fn stale_preparation_and_failed_backup_preserve_the_current_database() {
    ...
    fs::write(model, content.replace("A fictional model.", "An updated model.")).unwrap();
    let changed = CanonicalSnapshot::scan(&workspace).unwrap();
    store.reconcile(&changed).unwrap();
    let expected = store.logical_snapshot().unwrap();
    drop(store);
    assert_eq!(candidate.publish(&snapshot).unwrap_err().code, "REBUILD_GENERATION_CHANGED");
    fs::write(temp.path().join(".knowmesh/backups"), "blocked backup directory").unwrap();
    assert_eq!(rebuild::execute(&workspace, &backend, &input).unwrap_err().code, "REBUILD_IO_FAILED");
