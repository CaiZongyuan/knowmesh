...
use knowmesh_core::{
    ...
    canonical::{snapshot::CanonicalSnapshot, source::{ImportInput, SourceLibrary}},
    domain::StorageMode,
...
struct InterruptedStore<'a>(&'a mut SqliteStore);

impl ProjectionStore for InterruptedStore<'_> {
    fn reconcile(&mut self, _: &CanonicalSnapshot) -> AppResult<ReconcileReport> {
        // The real store remains untouched, simulating failure before the SQLite commit.
        let _ = self.0.generation()?;
        Err(AppError::new(ErrorType::Io, "INJECTED_DATABASE_FAILURE", "Injected failure"))
    }
}
...
fn source_import_preview_commit_and_recovery_share_the_same_transaction_path() {
    ...
    input.dry_run = false;
    let mut interrupted = InterruptedStore(&mut store);
    assert_eq!(source::add(&workspace, &mut interrupted, &input, None).unwrap_err().code, "INJECTED_DATABASE_FAILURE");
    assert_eq!(store.generation().unwrap(), 1);
    ...
    assert_eq!(sync::synchronize(&workspace, &mut store).unwrap_err().code, "TRANSACTION_RECOVERY_REQUIRED");
    ...
    assert_eq!(store.generation().unwrap(), 2);
    let source_id = SourceLibrary::new(&workspace).list(true).unwrap().into_iter().find(|file| file.manifest.title == "Second source").unwrap().manifest.id;
    ...
    assert_eq!(store.generation().unwrap(), 2);
    source::remove(&workspace, &mut store, &source::RemoveInput { source_id: source_id.clone(), dry_run: false }).unwrap();
    let removed = SourceLibrary::new(&workspace).get(&source_id).unwrap();
    assert!(removed.manifest.removed_at.is_some());
    assert!(SourceLibrary::new(&workspace).content(&source_id, None).is_ok());
...
fn an_external_edit_after_interruption_preserves_the_journal_and_old_index() {
    ...
    let mut interrupted = InterruptedStore(&mut store);
    source::remove(&workspace, &mut interrupted, &source::RemoveInput { source_id: id, dry_run: false }).unwrap_err();
    let path = temp.path().join(&snapshot.sources[0].manifest_path);
    let content = std::fs::read_to_string(&path).unwrap() + "\n# External edit\n";
    std::fs::write(&path, &content).unwrap();
    assert_eq!(sync::recover(&workspace, &mut store).unwrap_err().code, "TRANSACTION_RECOVERY_CONFLICT");
