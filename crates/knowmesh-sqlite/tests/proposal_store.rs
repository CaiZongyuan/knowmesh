...
use knowmesh_core::{
    ...
    domain::{Timestamp, proposal::{PatchOp, ProposalInput, ProposalItem, ProposalKind, ReviewInput, ReviewPolicy}},
...
fn fixture() -> (tempfile::TempDir, Workspace, SqliteStore, ProposalRecord) {
    ...
    let prepared = prepare(&workspace, &ProposalInput {
        kind: ProposalKind::Manual, base_generation:generation, schema_hash:snapshot.schema_hash,
        source_revision_id:None, compiler_run_id:None, summary:"Stored proposal.".into(),
        items:vec![ProposalItem::new(PatchOp::AddAlias, snapshot.nodes[0].metadata.id.to_string(), json!({"alias":"Proposed alias"})).unwrap()],
    }, "author", now()).unwrap();
    let record = ProposalRecord { proposal:prepared.proposal, base_snapshot_sha256:prepared.base_snapshot_sha256 };
    (temp, workspace, store, record)
}

fn reviewed(record: &ProposalRecord) -> ProposalRecord {
    ProposalRecord {
        proposal:record.proposal.review(&ReviewInput { expected_revision:record.proposal.revision, accept_all:true, decisions:vec![] }, &ReviewPolicy::default(), "reviewer", now()).unwrap(),
...
fn revision_history_keeps_complete_review_metadata_and_atomic_current_items() {
    ...
    assert_eq!(store.proposal_get(&original.proposal.id, None).unwrap(), original);
    ...
    assert_eq!(db.query_row("SELECT count(*) FROM proposal_revisions", [], |r| r.get::<_, i64>(0)).unwrap(), 2);
    assert_eq!(db.query_row("SELECT count(*) FROM audit_events", [], |r| r.get::<_, i64>(0)).unwrap(), 2);
    assert_eq!(db.query_row("SELECT decision FROM proposal_items", [], |r| r.get::<_, String>(0)).unwrap(), "accepted");
    assert_eq!(CanonicalSnapshot::scan(&workspace).unwrap().content_sha256, original.base_snapshot_sha256);
    drop(store);
    let reopened = SqliteStore::open_read_only(&workspace.index_path().unwrap()).unwrap();
    assert_eq!(reopened.proposal_get(&original.proposal.id, Some(2)).unwrap(), next);
}
...
fn stale_writers_cannot_replace_a_newer_revision_or_skip_history() {
    ...
    let next = reviewed(&original);
    store.proposal_save(1, &next).unwrap();
    let rejected = ProposalRecord { proposal:stale.proposal.reject(1, "Rejected from stale read", "other-reviewer", now()).unwrap(), ..stale };
    assert_eq!(second.proposal_save(1, &rejected).unwrap_err().code, "PROPOSAL_REVISION_MISMATCH");
    let mut skipped = next.clone();
    skipped.proposal.revision = 4;
    assert_eq!(second.proposal_save(2, &skipped).unwrap_err().code, "PROPOSAL_REVISION_MISMATCH");
    assert_eq!(second.proposal_get(&original.proposal.id, None).unwrap(), next);
    assert_eq!(store.proposal_create(&original).unwrap_err().code, "PROPOSAL_ALREADY_EXISTS");
}
...
fn failed_runtime_write_rolls_back_header_items_history_and_audit_together() {
    ...
    assert_eq!(store.proposal_get(&original.proposal.id, None).unwrap(), original);
    assert_eq!(db.query_row("SELECT count(*) FROM proposal_revisions", [], |r| r.get::<_, i64>(0)).unwrap(), 1);
    assert_eq!(db.query_row("SELECT decision FROM proposal_items", [], |r| r.get::<_, String>(0)).unwrap(), "pending");
}
...
fn unindexed_baselines_and_direct_applied_metadata_cannot_be_saved() {
    ...
    assert_eq!(store.proposal_create(&stale).unwrap_err().code, "STALE_PROPOSAL");
    store.proposal_create(&original).unwrap();
    let next = reviewed(&original);
    store.proposal_save(1, &next).unwrap();
    let applied = ProposalRecord { proposal:next.proposal.mark_applied(2, 2, "author", now()).unwrap(), ..next.clone() };
    assert_eq!(store.proposal_save(2, &applied).unwrap_err().code, "PROPOSAL_APPLY_COORDINATOR_REQUIRED");
    assert_eq!(store.proposal_get(&original.proposal.id, None).unwrap(), next);
}
...
fn corrupt_snapshot_content_and_header_pointers_are_detected_on_read() {
    ...
    store.proposal_create(&original).unwrap();
    let db = Connection::open(workspace.index_path().unwrap()).unwrap();
    db.execute("UPDATE proposal_revisions SET snapshot_json='{}'", []).unwrap();
    assert_eq!(store.proposal_get(&original.proposal.id, None).unwrap_err().code, "PROPOSAL_HISTORY_INVALID");
...
fn runtime_copy_retains_every_proposal_revision_and_review_binding() {
    ...
    candidate.bind_workspace(&workspace.config.workspace.id, &snapshot.schema_hash).unwrap();
    ...
    assert_eq!(candidate.proposal_get(&original.proposal.id, Some(1)).unwrap(), original);
    assert_eq!(candidate.proposal_get(&original.proposal.id, None).unwrap(), next);
