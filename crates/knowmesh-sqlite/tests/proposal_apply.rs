...
use knowmesh_core::{
    application::{proposal::{ProposalRecord, apply::{self, ApplyInput}, prepare}, sync},
    canonical::{node::NodeDocument, snapshot::CanonicalSnapshot, workspace::Workspace},
    domain::{Timestamp, proposal::{PatchOp, ProposalInput, ProposalItem, ProposalKind, ProposalState, ReviewInput, ReviewPolicy}},
...
fn now() -> Timestamp { "2026-09-06T02:00:00Z".parse().unwrap() }

fn fixture() -> (tempfile::TempDir, Workspace, SqliteStore, ProposalRecord, ApplyInput) {
    ...
    let prepared = prepare(&workspace, &ProposalInput {
        ...
        source_revision_id:None, compiler_run_id:None, summary:"Add two reviewed aliases.".into(),
        items:snapshot.nodes.iter().map(|node| ProposalItem::new(PatchOp::AddAlias, node.metadata.id.to_string(), json!({"alias":format!("Reviewed {}",node.metadata.name)})).unwrap()).collect(),
    }, "author", now()).unwrap();
...
fn apply_commits_canonical_index_review_history_and_a_repeatable_receipt() {
    ...
    assert_eq!(store.proposal_get(&input.proposal_id, Some(2)).unwrap(), approved);
    let snapshot = CanonicalSnapshot::scan(&workspace).unwrap();
    assert!(snapshot.nodes.iter().all(|node| node.metadata.aliases.contains(&format!("Reviewed {}", node.metadata.name))));
    ...
    assert_eq!(serde_json::to_value(&repeated).unwrap(), serde_json::to_value(&report).unwrap());
    let db = Connection::open(workspace.index_path().unwrap()).unwrap();
    assert_eq!(db.query_row("SELECT count(*) FROM proposal_revisions", [], |row| row.get::<_, u64>(0)).unwrap(), 3);
    assert_eq!(db.query_row("SELECT count(*) FROM proposal_applications", [], |row| row.get::<_, u64>(0)).unwrap(), 1);
}
...
fn apply_preview_and_missing_confirmation_do_not_mutate_runtime_or_canonical() {
    ...
    input.yes = false;
    assert_eq!(apply::execute(&workspace, &mut store, &input, "author", now()).unwrap_err().code, "CONFIRMATION_REQUIRED");
    input.dry_run = true;
    let report = apply::execute(&workspace, &mut store, &input, "author", now()).unwrap();
    assert!(report.dry_run);
    assert_eq!(report.changed_paths.len(), 2);
    ...
    assert_eq!(store.proposal_get(&input.proposal_id, None).unwrap(), approved);
    assert_eq!(CanonicalSnapshot::scan(&workspace).unwrap().content_sha256, before.content_sha256);
    assert!(store.proposal_application(&input.proposal_id).unwrap().is_none());
}
...
fn stale_content_and_wrong_revisions_fail_before_any_file_journal() {
    ...
    assert_eq!(apply::execute(&workspace, &mut store, &input, "author", now()).unwrap_err().code, "PROPOSAL_REVISION_MISMATCH");
    ...
    assert_eq!(apply::execute(&workspace, &mut store, &input, "author", now()).unwrap_err().code, "STALE_PROPOSAL");
    assert!(!sync::recovery_status(&workspace).unwrap().recovery_required);
    assert!(store.proposal_application(&input.proposal_id).unwrap().is_none());
}
...
fn failure_after_file_changes_rolls_back_sql_and_recovers_the_reviewed_revision() {
    ...
    assert!(sync::recovery_status(&workspace).unwrap().recovery_required);
    assert_eq!(store.proposal_get(&input.proposal_id, None).unwrap(), approved);
    assert_eq!(db.query_row("SELECT indexed_generation FROM workspace_state", [], |row| row.get::<_, u64>(0)).unwrap(), 1);
    assert!(store.proposal_application(&input.proposal_id).unwrap().is_none());
    for filename in ["model-a.md", "dataset-b.md"] {
        assert!(fs::read_to_string(workspace.root.join("knowledge/nodes").join(filename)).unwrap().contains("Reviewed "));
    }
    ...
    assert_eq!(store.proposal_get(&input.proposal_id, None).unwrap().proposal.state, ProposalState::Applied);
    let repeated = apply::execute(&workspace, &mut store, &input, "retry", now()).unwrap();
...
fn recovery_refuses_to_apply_a_journal_after_its_reviewed_revision_changes() {
    ...
    assert!(apply::execute(&workspace, &mut store, &input, "author", now()).is_err());
    db.execute_batch("DROP TRIGGER fail_apply_history;").unwrap();
    let rejected = ProposalRecord { proposal:approved.proposal.reject(2, "Review changed during interruption", "reviewer", now()).unwrap(), ..approved };
    store.proposal_save(2, &rejected).unwrap();
    assert_eq!(sync::recover(&workspace, &mut store).unwrap_err().code, "PROPOSAL_REVISION_MISMATCH");
    assert!(sync::recovery_status(&workspace).unwrap().recovery_required);
    assert_eq!(store.proposal_get(&input.proposal_id, None).unwrap(), rejected);
    assert_eq!(db.query_row("SELECT indexed_generation FROM workspace_state", [], |row| row.get::<_, u64>(0)).unwrap(), 1);
}
