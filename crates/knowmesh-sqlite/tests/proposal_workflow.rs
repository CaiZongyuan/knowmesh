...
use knowmesh_core::{
    ...
    canonical::{
        node::NodeDocument, schema::Schema, snapshot::CanonicalSnapshot, workspace::Workspace,
    },
    domain::{
        Timestamp,
        proposal::{
            Decision, PatchOp, ProposalInput, ProposalItem, ProposalRevision, ProposalState,
            ReviewInput,
        },
    },
...
fn fixture() -> (tempfile::TempDir, Workspace, SqliteStore, CreateInput) {
    ...
    let input = CreateInput {
        ...
        proposal: ProposalInput {
            kind: knowmesh_core::domain::proposal::ProposalKind::Manual,
            base_generation: generation,
            schema_hash: snapshot.schema_hash,
            source_revision_id: None,
            compiler_run_id: None,
            summary: "Review workflow changes.".into(),
            items: snapshot
                .nodes
                .iter()
                .map(|node| {
                    ProposalItem::new(
                        ...
                        json!({"alias":format!("Workflow {}",node.metadata.name)}),
                    )
                    .unwrap()
                })
                .collect(),
...
fn create_review_and_apply_share_validated_records_and_keep_history_readable() {
    ...
    assert!(
        created
            ...
            .items
            .iter()
            .all(|item| item.before_sha256.is_some())
    );
...
fn every_runtime_mutation_has_a_read_only_preview() {
    ...
    assert_eq!(
        store
            .proposal_get(&preview.record.proposal.id, None)
            .unwrap_err()
            .code,
        "PROPOSAL_NOT_FOUND"
    );
    ...
    .unwrap();
    assert_eq!(reviewed.record.proposal.state, ProposalState::Approved);
    let rejected = workflow::reject(
        &workspace,
        &mut store,
        &RejectInput {
            proposal_id: created.proposal.id.clone(),
            expected_revision: 1,
            reason: "Preview rejection".into(),
            dry_run: true,
        },
        "author",
        now(),
    ...
    let edited = workflow::edit(
        ...
        &EditInput {
            ...
            revision: ProposalRevision {
                expected_revision: 1,
                base_generation: 1,
                schema_hash: created.proposal.schema_hash.clone(),
                summary: created.proposal.summary.clone(),
                items,
            },
        },
        "editor",
    ...
    assert_eq!(
        store.proposal_get(&created.proposal.id, None).unwrap(),
        created
    );
    assert_eq!(
        CanonicalSnapshot::scan(&workspace).unwrap().content_sha256,
        created.base_snapshot_sha256
    );
}
...
fn editing_revalidates_payloads_and_resets_only_changed_item_reviews() {
    ...
    let mut edit = EditInput {
        proposal_id: approved.proposal.id.clone(),
        dry_run: false,
        revision: ProposalRevision {
            expected_revision: 2,
            base_generation: 1,
            schema_hash: approved.proposal.schema_hash.clone(),
            summary: approved.proposal.summary.clone(),
            items,
        },
    };
    let invalid = workflow::edit(&workspace, &mut store, &edit, "editor", now())
        .unwrap()
        .record;
    ...
    let repaired = workflow::edit(&workspace, &mut store, &edit, "editor", now())
        .unwrap()
        .record;
    assert!(repaired.proposal.items[0].issues.is_empty());
    assert_eq!(repaired.proposal.items[1].decision, Decision::Accepted);
    assert_eq!(
        store.proposal_get(&created.proposal.id, Some(2)).unwrap(),
        approved
    );
}
...
fn stale_review_requires_explicit_revalidation_and_all_previous_reviews_reset() {
    ...
    let preview = workflow::revalidate(
        &workspace,
        &mut store,
        &RevalidateInput {
            proposal_id: created.proposal.id.clone(),
            expected_revision: stale.proposal.revision,
            dry_run: true,
        },
        "editor",
        now(),
    ...
    let revised = workflow::revalidate(
        &workspace,
        &mut store,
        &RevalidateInput {
            proposal_id: created.proposal.id.clone(),
...
fn workspace_policy_is_enforced_and_stale_proposals_can_be_explicitly_rejected() {
    ...
    let rejected = workflow::reject(
        ...
        &RejectInput {
            ...
            expected_revision: 1,
            reason: "Reject obsolete draft".into(),
            dry_run: false,
        },
        "author",
        now(),
...
#[test]
fn pending_apply_journals_block_authoring_but_preserve_history_reads() {
    let (_temp, workspace, mut store, input) = fixture();
    let created = workflow::create(&workspace, &mut store, &input, "author", now())
        .unwrap()
        .record;
    ...
    db.execute_batch("CREATE TRIGGER fail_apply BEFORE INSERT ON proposal_revisions WHEN NEW.revision=3 BEGIN SELECT RAISE(ABORT,'injected failure'); END;").unwrap();
    apply::execute(
        &workspace,
        &mut store,
        &ApplyInput {
    ...
    assert_eq!(
        workflow::reject(
            ...
            &RejectInput {
                proposal_id: created.proposal.id.clone(),
                expected_revision: 2,
                reason: "Do not change an interrupted Apply".into(),
                dry_run: false
            },
            "author",
            now()
