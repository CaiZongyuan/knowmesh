...
fn rebuild_cli_exposes_confirmation_preview_and_backup_retention() {
    ...
    assert_eq!(serde_json::from_slice::<Value>(&preview.stdout).unwrap()["data"]["dry_run"], true);
    ...
    assert_eq!(std::fs::read_dir(root.join(".knowmesh/backups")).unwrap().count(), 1);
    ...
    assert_eq!(serde_json::from_slice::<Value>(&invalid.stderr).unwrap()["error"]["code"], "INVALID_BACKUP_RETENTION");
}
