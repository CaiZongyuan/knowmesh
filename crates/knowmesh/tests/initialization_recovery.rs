...
fn interrupted(root: &Path, applied: usize) -> (PathBuf, Vec<(PathBuf, Vec<u8>)>) {
    ...
    let directory = fs::read_dir(root.join(".knowmesh/transactions"))
        .unwrap()
        .next()
        .unwrap()
        .unwrap()
...
fn doctor_recovers_initialization_before_configuration_exists_at_every_file_boundary() {
    for applied in 0..=5 {
        ...
        assert_eq!(
            error(invoke(&root, &["doctor", "--repair"])),
        ...
        assert_eq!(
            preview["recovery"]["transactions"][0]["paths"]
                .as_array()
                .unwrap()
                .len(),
            files.len()
        ...
        for (index, (path, _)) in files.iter().enumerate() {
            assert_eq!(path.exists(), index < applied);
        }
        ...
        assert_eq!(
            report["recovery"]["recovered_transaction_ids"]
                .as_array()
                .unwrap()
                .len(),
            1
        ...
        assert_eq!(
            success(invoke(&root, &["doctor", "--repair", "--yes"]))["generation"],
            1
        );
    }
}
...
fn recovery_preview_and_execution_preserve_external_edits_and_bad_staging() {
    for corrupt_staging in [false, true] {
        ...
        let target = if corrupt_staging {
            staging.join("1.blob")
        } else {
            files[0].0.clone()
        };
        fs::write(&target, "External changes.").unwrap();
        let code = if corrupt_staging {
...
fn configuration_recovery_rejects_another_workspaces_database_before_writing() {
    ...
    fs::copy(
        other.join(".knowmesh/index.sqlite3"),
        root.join(".knowmesh/index.sqlite3"),
    )
    .unwrap();
    assert_eq!(
        error(invoke(&root, &["doctor", "--repair", "--yes"])),
...
#[test]
fn doctor_resolves_recovery_from_environment_and_ancestors_and_repairs_invalid_config() {
    let temp = tempfile::tempdir().unwrap();
    let root = temp.path().join("workspace");
    let (_, files) = interrupted(&root, 5);
    let config_path = root.join("knowmesh.yaml");
    let invalid = b"workspace: [broken";
    fs::write(&config_path, invalid).unwrap();
    let journal = fs::read_dir(root.join(".knowmesh/transactions"))
        .unwrap()
        .next()
        .unwrap()
        .unwrap()
        ...
        .join("manifest.json");
    let mut manifest: Value = serde_json::from_slice(&fs::read(&journal).unwrap()).unwrap();
    let config = manifest["changes"]
        .as_array_mut()
        .unwrap()
        .iter_mut()
        .find(|change| change["path"] == "knowmesh.yaml")
        .unwrap();
    config["operation"] = "replace".into();
    config["before_sha256"] = knowmesh_core::domain::sha256(invalid).into();
    fs::write(journal, serde_json::to_vec(&manifest).unwrap()).unwrap();
    let report = success(
        cargo_bin_cmd!("knowmesh")
    ...
    assert_eq!(report["recovery"]["recovery_required"], true);
    success(
        cargo_bin_cmd!("knowmesh")
            .current_dir(temp.path())
            .env("KNOWMESH_WORKSPACE", &root)
            .args(["doctor", "--repair", "--yes"])
            .output()
            .unwrap(),
    );
    let expected = files.iter().find(|(path, _)| path == &config_path).unwrap();
    assert_eq!(fs::read(&config_path).unwrap(), expected.1);
}
