...
fn source_impact_cli_uses_the_registered_read_contract() {
    ...
    let added = cargo_bin_cmd!("knowmesh")
        .arg("--workspace")
        .arg(&root)
        .args(["source", "add"])
        .arg(path)
    ...
    let result = cargo_bin_cmd!("knowmesh")
        .arg("--workspace")
        .arg(&root)
        .args(["source", "impact", id, "--limit", "1"])
        .output()
        .unwrap();
    assert!(
    ...
    let schema = cargo_bin_cmd!("knowmesh")
        .args(["schema", "command", "source.impact"])
        .output()
        .unwrap();
    assert!(schema.status.success());
    assert_eq!(
        serde_json::from_slice::<Value>(&schema.stdout).unwrap()["data"]["effect"],
        "read"
    );
}
