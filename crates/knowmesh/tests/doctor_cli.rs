...
fn doctor(root: &Path) -> Value {
    let output = cargo_bin_cmd!("knowmesh").arg("--workspace").arg(root).arg("doctor").output().unwrap();
...
fn doctor_reports_missing_stale_and_invalid_content_without_changing_the_index() {
    ...
    assert!(initial["issues"].as_array().unwrap().iter().any(|i| i["code"] == "INDEX_MISSING"));
    assert!(!root.join(".knowmesh/index.sqlite3").exists());
    assert!(Command::new("git").arg("-C").arg(&root).args(["init", "-q"]).status().unwrap().success());
    cargo_bin_cmd!("knowmesh").arg("--workspace").arg(&root).arg("sync").assert().success();
    ...
    assert!(invalid["issues"].as_array().unwrap().iter().any(|i| i["severity"] == "error"));
}
...
fn doctor_preserves_a_corrupt_database_and_requires_confirmation_for_repair() {
    ...
    let root = temp.path().join("workspace");
    cargo_bin_cmd!("knowmesh").arg("init").arg(&root).assert().success();
    let unconfirmed = cargo_bin_cmd!("knowmesh").arg("--workspace").arg(&root).args(["doctor", "--repair"]).output().unwrap();
    ...
    assert!(report["issues"].as_array().unwrap().iter().any(|i| i["code"] == "DATABASE_CORRUPT"));
    assert_eq!(fs::read(root.join(".knowmesh/index.sqlite3")).unwrap(), b"corrupt fixture");
}
