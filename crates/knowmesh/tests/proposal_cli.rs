...
use knowmesh_core::{
    ...
    domain::{
        Timestamp,
        proposal::{PatchOp, ProposalItem, ProposalKind},
    },
};
...
fn call(root: &Path, args: &[&str], input: Option<Value>) -> std::process::Output {
    ...
    if let Some(input) = input {
        command.write_stdin(serde_json::to_vec(&input).unwrap());
    }
...
fn proposal_cli_creates_reviews_and_applies_json_from_stdin() {
    ...
    let historical = success(call(
        &workspace.root,
        &["proposal", "get", id, "--revision", "1"],
        None,
    ));
    assert_eq!(historical["data"]["proposal"]["state"], "draft");
    let unconfirmed = call(
        &workspace.root,
        &["proposal", "apply", id, "--expected-revision", "2"],
        None,
    );
    ...
    let applied = success(call(
        &workspace.root,
        &["proposal", "apply", id, "--expected-revision", "2", "--yes"],
        None,
    ));
    assert_eq!(applied["data"]["projection"]["generation"], 2);
    assert_eq!(
        success(call(&workspace.root, &["proposal", "get", id], None))["data"]["proposal"]["state"],
        "applied"
    );
}
...
fn proposal_request_files_match_descriptors_and_all_commands_are_discoverable() {
    ...
    success(call(&workspace.root, &["sync"], None));
    for (name, effect) in [
        ("proposal.create", "runtime-write"),
        ("proposal.get", "read"),
        ("proposal.review", "runtime-write"),
        ("proposal.edit", "runtime-write"),
        ("proposal.revalidate", "runtime-write"),
        ("proposal.reject", "runtime-write"),
        ("proposal.apply", "canonical-write"),
    ] {
        let descriptor = success(call(&workspace.root, &["schema", "command", name], None));
        assert_eq!(descriptor["data"]["effect"], effect);
    }
    let patch = success(call(
        &workspace.root,
        &["schema", "patch", "create_node"],
        None,
    ));
    ...
    let created = success(call(
        &workspace.root,
        &["proposal", "create", "--input", path.to_str().unwrap()],
        None,
    ));
    assert_eq!(
        created["data"]["record"]["proposal"]["created_by"],
        "human_cli"
    );
    let _: Timestamp = created["data"]["record"]["proposal"]["created_at"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
