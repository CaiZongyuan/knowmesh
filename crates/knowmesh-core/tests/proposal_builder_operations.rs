...
use knowmesh_core::{
    ...
    domain::{
        ...
        EvidenceId, EvidenceStance, EvidenceStatus, LifecycleStatus, NodeId, RelationId,
        SourceId, SynthesisId, Timestamp,
...
fn new_nodes_are_created_before_relations_and_source_metadata_dependencies() {
    ...
    let model = before.nodes.iter().find(|node| node.metadata.node_type == "Model").unwrap();
    let mut node = before.nodes.iter().find(|node| node.metadata.node_type == "Dataset").unwrap().metadata.clone();
    node.id = NodeId::new();
    ...
    let prepared = build(&workspace, vec![
        item(PatchOp::AddRelation, &model.metadata.id, json!({"relation":relation})),
        item(PatchOp::UpdateSourceMetadata, &source.id, json!({
        ...
        item(PatchOp::CreateNode, &node.id, json!({"metadata":node, "summary":"A new dataset."})),
    ]);
    unblocked(&prepared);
    ...
    assert_eq!(after.sources()[0].manifest.current_revision_id, source.current_revision_id);
    assert_eq!(after.sources()[0].manifest.storage, source.storage);
    assert_eq!(after.sources()[0].manifest.represented_nodes, vec![node.id.clone()]);
    let created = after.nodes().iter().find(|entry| entry.metadata.id == node.id).unwrap();
    ...
    for event in pulldown_cmark::Parser::new(document.body()) {
        match event {
            pulldown_cmark::Event::Start(pulldown_cmark::Tag::Heading { level, .. }) => heading = Some((level, String::new())),
            pulldown_cmark::Event::Text(text) => {
                if let Some((_, value)) = &mut heading { value.push_str(&text); }
            }
            pulldown_cmark::Event::End(pulldown_cmark::TagEnd::Heading(_)) => headings.push(heading.take().unwrap()),
            pulldown_cmark::Event::InlineHtml(_) => panic!("Node title injected HTML"),
            _ => {}
        }
    }
    assert_eq!(headings, vec![
        (pulldown_cmark::HeadingLevel::H1, "Dataset [C] <script> ## Notes".into()),
        (pulldown_cmark::HeadingLevel::H2, "Summary".into()),
    ]);
    assert_eq!(CanonicalSnapshot::scan(&workspace).unwrap().content_sha256, before.content_sha256);
...
fn lifecycle_edits_preserve_old_assertions_and_accept_new_replacements() {
    ...
    replacement_relation.qualifiers.insert("scope".into(), json!("new evaluation"));
    ...
    let prepared = build(&workspace, vec![
        item(PatchOp::SupersedeClaim, &claim.assertion.id, json!({"replacement_id":replacement_claim.id})),
        item(PatchOp::SupersedeRelation, &relation.assertion.id, json!({"replacement_id":replacement_relation.id})),
        item(PatchOp::RetractClaim, &retract_claim.id, json!({"reason":"Withdraw the candidate."})),
        item(PatchOp::RetractRelation, &retract_relation.id, json!({"reason":"Withdraw the candidate."})),
        item(PatchOp::AddClaim, &claim.subject_node_id, json!({"claim":replacement_claim})),
        item(PatchOp::AddRelation, &relation.source_node_id, json!({"relation":replacement_relation})),
        item(PatchOp::AddClaim, &claim.subject_node_id, json!({"claim":retract_claim})),
        item(PatchOp::AddRelation, &relation.source_node_id, json!({"relation":retract_relation})),
    ]);
    unblocked(&prepared);
    let after = prepared.preview.as_ref().unwrap();
    let old = after.claims().iter().find(|entry| entry.claim.assertion.id == claim.assertion.id).unwrap();
    ...
    let old = after.relations().iter().find(|entry| entry.relation.assertion.id == relation.assertion.id).unwrap();
    let mut expected = relation.assertion.clone();
    expected.lifecycle_status = LifecycleStatus::Superseded;
    assert_eq!(old.relation.assertion, expected);
    assert_eq!(after.claims().iter().find(|entry| entry.claim.assertion.id == retract_claim.id).unwrap().claim.assertion.lifecycle_status, LifecycleStatus::Retracted);
    assert_eq!(after.relations().iter().find(|entry| entry.relation.assertion.id == retract_relation.id).unwrap().relation.assertion.lifecycle_status, LifecycleStatus::Retracted);
}
...
fn evidence_append_preserves_identity_and_marks_opposed_stances_conflicting() {
    ...
    let prepared = build(&workspace, vec![
        item(PatchOp::AddEvidence, &before.claims[0].claim.assertion.id, json!({"evidence":[original, additional]})),
        item(PatchOp::AddEvidence, &before.relations[0].relation.assertion.id, json!({"evidence":[original, additional]})),
    ]);
    unblocked(&prepared);
    let after = prepared.preview.as_ref().unwrap();
    assert_eq!(after.claims()[0].claim.assertion.evidence, vec![original.clone(), additional.clone()]);
    assert_eq!(after.relations()[0].relation.assertion.evidence, vec![original.clone(), additional]);
    assert_eq!(after.claims()[0].claim.assertion.evidence_status, EvidenceStatus::Conflicting);
    assert_eq!(after.relations()[0].relation.assertion.evidence_status, EvidenceStatus::Conflicting);
}
...
fn conflict_groups_depend_on_new_claims_without_hiding_group_copies_in_them() {
    ...
    let group = ConflictGroup {
        id: ConflictGroupId::new(), claim_ids, reason:"Conflicting evaluation statements.".into(),
        status: ConflictGroupStatus::Open, created_at: now(), resolved_at: None,
    };
    let group_item = item(PatchOp::RecordClaimConflict, &claim.subject_node_id, json!({"group":group}));
    let prepared = build(&workspace, vec![group_item.clone(), item(PatchOp::AddClaim, &claim.subject_node_id, json!({"claim":other}))]);
    unblocked(&prepared);
    assert!(prepared.proposal.items[1].payload["claim"].get("conflict_groups").is_none());
    for claim in prepared.preview.as_ref().unwrap().claims() {
        assert_eq!(claim.claim.assertion.conflict_groups, vec![group.clone()]);
        assert_eq!(claim.claim.assertion.evidence_status, EvidenceStatus::Conflicting);
    }
...
fn synthesis_preserves_supplied_historical_dependency_hashes_and_source_heads() {
    ...
    let AssertionDependency::Claim { semantic_sha256, .. } = &mut metadata.dependency_snapshot.as_mut().unwrap().assertions[0] else { unreachable!() };
    *semantic_sha256 = "a".repeat(64);
    let frozen = metadata.dependency_snapshot.clone();
    let prepared = build(&workspace, vec![item(PatchOp::CreateSynthesis, &metadata.id, json!({
        "metadata":metadata, "body":format!("# New synthesis\n\nA cited answer. [@{}]", metadata.evidence_ids[0])
    }))]);
    unblocked(&prepared);
    assert_eq!(prepared.proposal.items[0].payload["metadata"]["dependency_snapshot"], serde_json::to_value(&frozen).unwrap());
    assert_eq!(prepared.preview.as_ref().unwrap().syntheses().iter().find(|entry| entry.metadata.id == metadata.id).unwrap().metadata.dependency_snapshot, frozen);
}
...
fn synthesis_requires_available_schema_and_complete_referentially_valid_snapshots() {
    ...
    for (variant, code) in [
        ...
        match variant {
            ...
            2 => metadata.dependency_snapshot.as_mut().unwrap().assertions[0] = AssertionDependency::Claim { id: ClaimId::new(), semantic_sha256:"a".repeat(64) },
            3 => metadata.dependency_snapshot.as_mut().unwrap().source_heads[0].source_id = SourceId::new(),
            4 => metadata.dependency_snapshot.as_mut().unwrap().source_heads.clear(),
            _ => unreachable!(),
        }
        let prepared = build(&workspace, vec![item(PatchOp::CreateSynthesis, &metadata.id, json!({
...
fn evidence_overflow_returns_a_blocked_item_without_exceeding_review_metadata_limits() {
    let (_temp, workspace) = support::fixture();
    let before = CanonicalSnapshot::scan(&workspace).unwrap();
    let evidence: Vec<_> = (0..1025).map(|_| {
    ...
    assert!(prepared.documents().is_empty());
    assert_eq!(prepared.proposal.items[0].payload["evidence"].as_array().unwrap().len(), 1025);
}
...
fn full_warning_lists_still_allow_builder_to_block_invalid_payloads() {
    let (_temp, workspace) = support::fixture();
    let before = CanonicalSnapshot::scan(&workspace).unwrap();
    let mut invalid = item(PatchOp::AddAlias, &before.nodes[0].metadata.id, json!({"unknown":"field"}));
    invalid.issues = (0..128).map(|index| ProposalIssue { code:format!("UPSTREAM_WARNING_{index}"), message:"An upstream warning.".into(), blocking:false }).collect();
...
fn source_metadata_cannot_modify_revision_storage_or_unrelated_identity_fields() {
    ...
    let before = CanonicalSnapshot::scan(&workspace).unwrap();
    let source = &before.sources[0].manifest;
    for field in ["revisions", "current_revision_id", "removed_at", "storage", "id", "path"] {
        ...
        let prepared = build(&workspace, vec![item(PatchOp::UpdateSourceMetadata, &source.id, payload)]);
...
fn conflict_group_evidence_is_verified_without_expanding_item_reference_metadata() {
    ...
    claim_ids.sort();
    let group = ConflictGroup {
        id:ConflictGroupId::new(), claim_ids, reason:"Conflicting statements.".into(),
        status:ConflictGroupStatus::Open, created_at:now(), resolved_at:None,
    };
    let prepared = build(&workspace, vec![item(PatchOp::RecordClaimConflict, owner, json!({"group":group, "member_statuses": BTreeMap::<ClaimId,EvidenceStatus>::new()}))]);
    unblocked(&prepared);
