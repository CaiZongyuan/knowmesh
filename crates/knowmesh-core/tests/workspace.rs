...
fn initialize_creates_portable_canonical_files_and_is_idempotent() {
    ...
    let options = InitOptions { name: "Virtual Cell".into(), template: "research".into(), dry_run: false };
    ...
    assert!(fs::read_to_string(root.join(".gitignore")).unwrap().lines().any(|line| line == ".knowmesh/"));
...
fn dry_run_and_conflicting_initialization_do_not_overwrite_user_data() {
    ...
    let mut options = InitOptions { name: "Research".into(), template: "research".into(), dry_run: true };
    ...
    assert_eq!(fs::read_to_string(root.join("purpose.md")).unwrap(), "Human-authored research scope");
}
...
fn workspace_resolution_obeys_explicit_then_environment_then_ancestors() {
    ...
    assert_eq!(resolve_workspace(None, None, &nested).unwrap(), fs::canonicalize(&a).unwrap());
    assert_eq!(resolve_workspace(None, Some(b.as_path()), &nested).unwrap(), fs::canonicalize(&b).unwrap());
    assert_eq!(resolve_workspace(Some(a.as_path()), Some(b.as_path()), &nested).unwrap(), fs::canonicalize(&a).unwrap());
    let missing = temp.path().join("missing");
    assert_eq!(resolve_workspace(Some(&missing), Some(&b), &nested).unwrap_err().code, "WORKSPACE_NOT_FOUND");
...
fn future_config_versions_are_rejected_before_use() {
    ...
    assert_eq!(Workspace::load(temp.path()).unwrap_err().code, "UNSUPPORTED_CONFIG_VERSION");
}
...
fn purpose_is_optional_but_configured_files_are_bounded_and_confined() {
    ...
    assert_eq!(Workspace::load(temp.path()).unwrap_err().code, "PATH_OUTSIDE_WORKSPACE");
    ...
    assert_eq!(Workspace::load(temp.path()).unwrap_err().code, "PURPOSE_TOO_LARGE");
}
...
fn model_environment_resolution_never_writes_or_formats_secrets() {
    ...
    env.insert("KNOWMESH_LLM_BASE_URL".into(), "https://example.invalid/v1".into());
    ...
    assert!(!fs::read_to_string(temp.path().join("knowmesh.yaml")).unwrap().contains("fixture-secret"));
...
fn configured_purpose_cannot_escape_through_a_symlink() {
    ...
    assert_eq!(Workspace::load(&root).unwrap_err().code, "PATH_OUTSIDE_WORKSPACE");
}
